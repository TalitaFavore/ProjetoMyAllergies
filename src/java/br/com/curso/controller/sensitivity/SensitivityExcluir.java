...
public class SensitivityExcluir extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
            ...
            if (dao.excluir(id)) {
                mensagem = "Sensibilidade excluída com sucesso!";
            } else{
                mensagem = "Problemas ao excluir Sensibilidade";
            }
            request.setAttribute("mensagem", mensagem);
            response.sendRedirect("SensitivityListar");
        } catch(Exception ex){
            System.out.println("Problemas no Servlet ao excluir Sensibilidade! Erro: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
